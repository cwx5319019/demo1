...
/*
	总结函数份文件写法：
		主函数已经其他一些入口函数都写在main.c中。
		其他功能函数全都写在其他的.c文件中。所有.c文件，只需要包含一个自定义的头文件，该头文件包含该程序所需要的所有系统头文件，已经所有自定义的函数的声明
	static 的另一个作用：限定作用域
	在一个.c文件中，我们如果想要引用另外一个.c文件中的全局变量的话，2中方式
	extern 类型 + 全局变量名   (它的作用和<>差不多，加上就必须再外面找，不加的话 先尝试再外面找，没有的话在看文件内部)
	或者没有extern
