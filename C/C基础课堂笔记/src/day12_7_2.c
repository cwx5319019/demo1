...
int bubble_price(fruit a,fruit b){
...
void custom_memory(fruit* fruitDB,store* storeDB,int len ,int* len_1){
	...
	mystore.salary = fruitDB[res].salary;
...
void custom_look_store(store* storeDB ,int len){
	...
	for(i = 0;i<len ; i++){
		...
		printf("水果名称：%s\n", storeDB[i].name);
		printf("水果重量：%.2f\n", storeDB[i].kilogram);
		printf("总   计：%.2f 元 \n", storeDB[i].salary * storeDB[i].kilogram);
		printf("***********************************\n");	
	}
}
...
void custom_bill(store* storeDB,int len){
	...
	float sum = 0.0;
	for(i = 0;i<len ;i++){
		sum += storeDB[i].salary * storeDB[i].kilogram;
	}
	printf("%f\n",sum);
}
...
int main(){
	int (*bubble[3])(fruit,fruit) = {bubble_name,bubble_kg,bubble_price};
	int len = 0;
	//adm_login();
